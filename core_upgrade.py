...
def update_item_availability(i, budget):
    ...
    if budget > 0 and 0 < item_price <= budget:
...
def calculate_budget(budget, labels, prices, base_quantity, limited_quantity):
    try:
        ...
        quantities = np.zeros(item_count, dtype=int)
        balances = np.zeros(item_count, dtype=int)
        ...
        total_budget = budget
        fixed_budget = np.sum(np.array(base_quantity) * np.array(prices))
        budget -= fixed_budget
        limits = np.array(limited_quantity) - np.array(base_quantity)
        complexity = np.prod(limits[:-1] + 1)
...
st.title("편리한 예산🍞만들기")
st.markdown('<p style="color: #a8a888;text-align: right;">SimBud(Budget Simulator V1.0)by 교사 박현수, 버그 및 개선 문의: <a href="mailto:hanzch84@gmail.com">hanzch84@gmail.com</a></p>', unsafe_allow_html=True)
